...
plugin.init = async function (data) {
	...
	data.router.get('/admin/plugins/composer-sejul', data.middleware.admin.buildHeader, controllers.renderAdminPage);
	data.router.get('/api/admin/plugins/composer-sejul', controllers.renderAdminPage);
};

plugin.appendConfig = async function (config) {
	config['composer-sejul'] = await meta.settings.get('composer-sejul');
	return config;
};

plugin.addAdminNavigation = async function (header) {
	header.plugins.push({
		route: '/plugins/composer-sejul',
		icon: 'fa-edit',
		name: 'Composer (sejul)',
	});
	return header;
